...
const idleAfterSeconds float32 = 1200
...
func TestReadsWorkspace(t *testing.T) {
	workspace := management.Workspace{
		...
		AutoSuspend: &struct {
			...
			IdleAfterSeconds: util.Ptr(idleAfterSeconds),
		},
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.singlestoredb_workspace.this", "auto_suspend.suspend_type", "IDLE"),
					resource.TestCheckResourceAttr("data.singlestoredb_workspace.this", "auto_suspend.suspend_after_seconds", fmt.Sprintf("%.0f", idleAfterSeconds)),
				),
			},
		},
	})
